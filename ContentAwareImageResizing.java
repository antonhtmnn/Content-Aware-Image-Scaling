...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ...
        int startNode = sx * sy;
        int endNode = sx * sy + 1;
        ShortestPathsTopological spt = new ShortestPathsTopological(graph, startNode);

        Stack<Integer> path = spt.pathTo(endNode);
        int size = path.size();
        int[] pathArray = new int[sy];
        int row = 0;
        for (int i = 0; i < size; i++) {
            int node = path.pop();
            if (node != startNode && node != endNode) {
                pathArray[row] = node % sx;
                row++;
            }
        }
        return pathArray;
    ...
    public static void main(String[] args) throws java.io.FileNotFoundException {
        demoPictureImage("/home/anton/Documents/AlgoDat/Blatt08/src/testImage.jpg");
        demoMatrixImage("/home/anton/Documents/AlgoDat/Blatt08/src/testMatrix.txt");
    }
}
